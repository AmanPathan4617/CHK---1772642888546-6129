...
def _gemini_generate(prompt: str):
    ...
    # Quality-focused instruction (no hard length cap)
    system_prompt = (
        "Provide a clear, helpful, and accurate answer. "
        "Prefer concise paragraphs and bullet points when useful."
    )
    body = {
        "generationConfig": {
            "temperature": 0.7,
  ...
  <body>
    <script>
      (function() {{
        ...
        var user = {json.dumps({"id": user.id, "username": user.username, "email": user.email})};
        try {{
