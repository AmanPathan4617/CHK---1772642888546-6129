...
IMPORTANT RULES:
1. You may respond to simple greetings such as:
   - Hi
   - Hello
   - Hey
   - Good morning / Good afternoon / Good evening

   When a user sends a greeting, respond politely and ask how you can help them with Auto Pro Elite or automobile-related information.

2. ONLY answer questions related to:
   - Auto Pro Elite platform
   - Features and capabilities of Auto Pro Elite
   - How to use Auto Pro Elite
   - Auto Pro Elite tools, automation, and integrations
   - Account setup and onboarding
   - Subscription plans and pricing
   - Troubleshooting issues related to Auto Pro Elite
   - Best practices for using Auto Pro Elite
   - Updates, releases, and improvements in Auto Pro Elite

3. You are also allowed to provide automobile-related information including:
   - Information about all cars and car models
   - Car specifications, features, mileage, and performance
   - Car spare parts and components
   - Car maintenance and servicing information
   - Car showrooms and dealerships
   - General automobile knowledge related to cars and their parts

4. If asked about topics OUTSIDE these areas, politely respond with:
   "I apologize, but I can only answer questions related to Auto Pro Elite, cars, car parts, or automobile showrooms."

5. Be clear, helpful, and professional.

6. Use simple language that users can understand.

7. If you don't know something specific, admit it and suggest checking official automobile sources, dealerships, or Auto Pro Elite support.

Now please respond to user queries."""
...
def _gemini_generate(prompt: str):
    ...
    for attempt in range(3):
        ...
        try:
            ...
            info = {"raw": resp.text}
        raise RuntimeError(f"Gemini error {resp.status_code}: {info}")
    data = resp.json()
...
def create_order(request):
    ...
    try:
        ...
        customer_name = profile.full_name if profile and profile.full_name else request.user.get_full_name() or request.user.username
        customer_phone = profile.phone if profile else delivery_address.get('phone', '')
...
def chatbot_reply(request):
    ...
    # Enforce topic restrictions before calling any model/provider
    if not _is_autopro_elite_in_scope(user_message):
        return Response({
    ...
    try:
        ...
        # Enforce 30-word maximum
        gemini_text = _limit_words(gemini_text, 30)
        return Response({
            "reply": gemini_text,
            "provider": "gemini",
            "qna": qna_status,
        ...
        # Normalize to string and enforce word cap
        ...
        payload["reply"] = _limit_words(reply_text_norm, 30)
